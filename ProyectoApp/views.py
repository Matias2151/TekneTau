...
def crear_proyecto(request):
    ...
    if cliente_id:
        try:
            ...
            proyecto.cliente = EmpresaPersona.objects.get(pk=cliente_id, emppe_est=True)
        except EmpresaPersona.DoesNotExist:
            if es_ajax(request):
                return JsonResponse(
                    {"success": False, "errors": {"cliente": ["El cliente seleccionado no existe o está inhabilitado."]}},
                    status=400,
                )
            messages.error(request, "El cliente seleccionado no existe o está inhabilitado.")
            return redirect("proyectoapp:lista_proyectos")
...
def editar_proyecto(request, pk):
    ...
    if cliente_id:
        try:
            proyecto.emppe_nom = EmpresaPersona.objects.get(pk=cliente_id, emppe_est=True)
        except EmpresaPersona.DoesNotExist:
            if es_ajax(request):
                return JsonResponse(
                    {"success": False, "errors": {"cliente": ["El cliente seleccionado no existe o está inhabilitado."]}},
